...
int main(int argc, char** argv) {
   ...
   int other_value;
   if (rank == 0) {
     printf("Processor %s, rank %d is now sending ..\n", processor_name, rank);
     MPI_Send(&local_value, 1, MPI_INT, 1, 0, MPI_COMM_WORLD);
     printf("Processor %s, rank %d is done sending!\n", processor_name, rank);
   }
   else {
     printf("Processor %s, rank %d is now receiving ..\n", processor_name, rank);
     MPI_Recv(&other_value, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
     printf("Processor %s, rank %d is done receiving! I have received %d\n", processor_name, rank, other_value);
   }
